...
async function onMonthChange(startOfMonth) {
  ...
  const endWeek = endOfMonth.week() === 1
...
function renderHeader({ startOfMonth }) {
  $header.querySelector('.calendar-month').innerText = startOfMonth.format('MMMM YYYY')
}

function renderCalendar({ startOfMonth, calendar, availability, selectedDate }) {
